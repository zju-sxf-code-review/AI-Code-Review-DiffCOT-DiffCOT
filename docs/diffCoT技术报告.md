...
## 三、项目技术选型的讨论与思考
直观地，根据项目任务书，我们首先自然地想到根据四种缺陷类型，提出四个Agent专门分别解决。

### 3.1 为什么没有按漏洞类型划分多Agent系统？

#### 3.1.1 背景

在项目初期的设计阶段，团队曾构思将PR审查任务按漏洞类型分为三类（静态缺陷、逻辑缺陷、安全及内存漏洞），并为每类漏洞配置专门的Agent进行审查，期望通过专业化分工提升检测性能。然而在深入分析后，我们发现这一架构面临多方面挑战，最终选择了**单Agent + 代码静态分析等工具调用**的架构方案。
...
**（2）多漏洞类型Agent知识库维护的开销与信息冗余**

Agent的专业性主要来源于两方面：**专属知识库**和**专属工具调用能力**。若采用多漏洞类型Agent架构：
