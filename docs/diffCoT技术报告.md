...
## 三、项目技术选型的讨论与思考
- [3.1 为什么不按漏洞类型划分多Agent系统？](#31-为什么不按漏洞类型划分多agent系统)
  - [3.1.1 背景](#311-背景)
  - [3.1.2 选择单Agent架构的考量理由](#312-选择单agent架构的考量理由)
- [3.2 针对实验Bad Case的解释与优化](#32-针对实验bad-case的解释与优化)
  - [3.2.1 上下文长度的挑战](#321-上下文长度的挑战)
  - [3.2.2 二次生成取并集策略](#322-二次生成取并集策略)
- [3.3 单Agent系统处理本任务的合理性论证](#33-单agent系统处理本任务的合理性论证)
...
### 3.1 为什么不按漏洞类型划分多Agent系统？

#### 3.1.1 背景

在项目初期的设计阶段，团队曾构思将PR审查任务按漏洞类型分为三类（静态缺陷、逻辑缺陷、安全及内存漏洞），并为每类漏洞配置专门的Agent进行审查，期望通过专业化分工提升检测性能。然而在深入分析后，我们发现这一架构面临多方面挑战，最终选择了**单Agent + 代码静态分析工具调用**的架构方案。
...
└─────────────────────────────┬───────────────────────────────────┘
              ...
              ▼               ▼               ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
│  Static Defect  │ │  Logic Defect   │ │    Security     │
...
#### 3.1.2 选择单Agent架构的考量理由

经过深入讨论，我们基于以下三点核心理由选择了单Agent架构：

**（1）漏洞类型划分的合理性有限**

在Benchmark数据集的50个案例分析中，我们观察到**大量错误呈现复合型特征**，难以被简单地归入单一类别。以下是具体佐证：

| PR案例 | 漏洞描述 | 涉及类型 |
|--------|----------|----------|
| SENTRY #2 | `Negative offset cursor manipulation bypasses pagination boundaries` | 逻辑缺陷 + 安全漏洞 |
| CAL.COM #4 | `Potential SQL injection risk in raw SQL query construction` | 安全漏洞 + 静态缺陷 |
| GRAFANA #4 | `Double interpolation risk` | 逻辑缺陷 + 安全漏洞 |
| KEYCLOAK #8 | `Wrong parameter in null check (grantType vs. rawTokenId)` | 静态缺陷 + 逻辑缺陷 |
| DISCOURSE #3 | `BlockedEmail.should_block? modifies DB during read` | 逻辑缺陷 + 代码异位 |

这些案例表明，真实世界的Bug往往**跨越多个类别边界**，若强行按Bug类型划分Agent，可能导致：

- 同一Bug被多个Agent重复检测，增加冗余
- 复合型Bug因信息分散而被各Agent遗漏
- Agent间需要大量协调通信，增加系统复杂度

**（2）多Agent知识库维护的开销与信息冗余**

Agent的专业性主要来源于两方面：**专属知识库**和**专属工具调用能力**。若采用多Agent架构：

```
知识库维护需求 = N个专属知识库 + 1个通用知识库
                = 4 + 1 = 5个知识库
```
...
| 问题类型 | 具体表现 |
|---------|---------|
| **知识分类困难** | 如"空指针检查"属于静态缺陷还是逻辑缺陷？分类错误将影响Agent表现 |
| **信息冗余** | 通用知识（如代码理解、意图分析）需同时注入所有Agent，造成重复 |
| **维护成本** | 知识更新需同步至多个库，一致性维护成本高 |
| **泛化依赖** | 若Agent被注入错误类型的知识，需依赖模型自身泛化能力，不可控 |

相比之下，单Agent架构只需维护**一套统一的知识库**，通过结构化的Prompt模板（如本项目的`review_rules.py`中包含的`HARD_EXCLUSION_RULES`、`STATIC_DEFECT_RULES`、`LOGIC_DEFECT_RULES`等）即可覆盖所有检测场景。

**（3）核心理念：专家深度思考优于流水线协作**

从实际效果角度，我们采用了一个朴素但有效的类比：

> **与其花同样的价格聘请4个人，每个人流水线地完成自己的任务再拼起来；不如多花点钱请一个经验充足的专家，一个人全部干完。剩下的预算可以再请另一个专家复核，两人讨论后得出最终结论。**

这一理念在本项目中的具体体现为：
- **单Agent专注思考**：一个Agent独立完成意图分析、静态检查、逻辑推理的完整流程，避免信息在Agent间传递时的损失
- **多工具并行辅助**：通过Semgrep、Tree-sitter、意图分析等工具提供多维度信息，由单一Agent综合判断
- **结果验证机制**：对于争议样本采用二次生成取并集的策略，模拟"两位专家讨论"的效果

### 3.2 针对实验Bad Case的解释与优化
...
### 3.3 单Agent系统处理本任务的合理性论证

基于对任务特性的分析，我们认为单Agent架构是本项目的合理选择：

#### （1）任务结构的顺序性

PR审查本质上是一个**顺序执行的任务流程**：
...
根据Agent的经典定义，一个Agent应具备**感知、决策、规划、执行**的能力。本项目的单Agent实现完全具备这些能力：
